...
function kartyaElrendezes(aktualisKartya) {
    ...
    ID("points").innerHTML =
        "<h3>Próbálkozások száma: " + probalkozasokSzama + "</h3>";

    for (let i = 0; i < tombHossz; i++)
        tomb[i].addEventListener("click", function () {
            kartyatMutat(i, aktualisKartya);
        });
}
...
function kartyatMutat(i, aktualisKartya) {
    if (szamlalo < 2 && event.target.className == "invisible") {
        ...
        if (szamlalo == 2) {
            if (
                aktualisKartya[kartyak[0]].kepEleresiUtvonala ==
                aktualisKartya[kartyak[1]].kepEleresiUtvonala
            ) {
                probalkozasokSzama++;
                ...
                setTimeout(() => {
                    ...
                    for (i = 0; i < 2; i++) {
                        ...
                        ID(Number(kartyak[i])).style.animation =
                            "megtalalt 1s 1";
...
function vegeKiir() {
    var szoveg =
        "Gratulálok! Sikeresen végigjátszotta a játékot!\n\nEnnyi próbálkozásból sikerült: " +
        probalkozasokSzama;
    alert(szoveg);
}
...
function init() {
    ID("title").innerHTML =
        "<a href='../html_oldalak/index.html'><h1>Fenntarthatósági témahét 2022</h1></a>";
    ...
    $a("#difficulty button")[0].className = "active";
